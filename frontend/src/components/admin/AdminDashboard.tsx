...
export function AdminDashboard() {
  ...
  return <main className="min-h-screen bg-slate-100 p-4 sm:p-8"><div className="mx-auto max-w-7xl space-y-8">
    ...
    <section className="rounded-2xl bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-xl font-black">Project work &amp; images</h2><button onClick={() => update('projects', [...content.projects, blankProject()])} className="text-sm font-bold text-amber-700">+ Add project</button></div><div className="mt-4 space-y-5">{content.projects.map((project, index) => <div key={project.id} className="rounded-xl border border-slate-200 p-4 grid gap-3 md:grid-cols-2"><input className={input} value={project.title} placeholder="Project title" onChange={(e) => { const next=[...content.projects];next[index]={...project,title:e.target.value};update('projects',next); }} /><select className={input} value={project.category} onChange={(e) => { const next=[...content.projects];next[index]={...project,category:e.target.value as typeof project.category};update('projects',next); }}><option>Roads</option><option>Public Works</option><option>Drainage</option><option>Earthwork</option><option>Buildings</option><option>Water Infrastructure</option></select><input className={input} value={project.location} placeholder="Location" onChange={(e) => { const next=[...content.projects];next[index]={...project,location:e.target.value};update('projects',next); }} /><input className={input} value={project.completionTime} placeholder="Completion time" onChange={(e) => { const next=[...content.projects];next[index]={...project,completionTime:e.target.value};update('projects',next); }} /><textarea className={input} value={project.description} placeholder="Short description" onChange={(e) => { const next=[...content.projects];next[index]={...project,description:e.target.value};update('projects',next); }} /><textarea className={input} value={project.fullDetails} placeholder="Full details" onChange={(e) => { const next=[...content.projects];next[index]={...project,fullDetails:e.target.value};update('projects',next); }} /><input className={input} value={project.image} placeholder="Image URL" onChange={(e) => { const next=[...content.projects];next[index]={...project,image:e.target.value};update('projects',next); }} /><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], index)} /><textarea className="md:col-span-2 rounded-lg border border-slate-300 px-3 py-2 text-sm" value={project.scope.join('\n')} placeholder="Scope items, one per line" onChange={(e) => { const next=[...content.projects];next[index]={...project,scope:e.target.value.split('\n').filter(Boolean)};update('projects',next); }} /><button onClick={() => update('projects', content.projects.filter((_, i) => i !== index))} className="w-fit text-xs font-bold text-red-700">Delete project</button></div>)}</div></section>
