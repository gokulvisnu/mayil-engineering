...
export function AdminDashboard() {
  ...
  async function deleteProject(projectIndex: number) {
    if (!content) return;
    const project = content.projects[projectIndex];
    if (!project || !window.confirm(`Delete â€œ${project.title || "this project"}â€? This cannot be undone.`)) return;
    if (isUploadedProjectImage(project.image)) {
      const response = await apiFetch("/api/admin/upload", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: project.image }) }, true);
  ...
  if (authorized !== true || !content) return <main className="min-h-screen grid place-items-center bg-slate-100 text-slate-700">Loading admin dashboardÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦</main>;
  ...
  return <main className="min-h-screen bg-slate-100 p-4 sm:p-8"><div className="mx-auto max-w-7xl space-y-8">
    ...
    <section className="rounded-2xl bg-white p-5 shadow-sm"><h2 className="text-xl font-black">Requested projects / enquiries</h2><div className="mt-4 space-y-3">{enquiries.length ? enquiries.map((entry)=><article key={entry.id} className="rounded-xl border p-4 text-sm"><div className="flex flex-wrap justify-between gap-2"><b>{entry.fullName}</b><select value={entry.status} onChange={(e)=>setInboxStatus('enquiry',entry.id,e.target.value)} className="rounded border px-2 py-1"><option value="new">New</option><option value="contacted">Contacted</option><option value="closed">Closed</option></select></div><p>{entry.projectType} ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {entry.location} ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {entry.phone}</p><p className="mt-1 text-slate-600">{entry.details}</p></article>) : <p className="text-sm text-slate-500">No enquiries yet.</p>}</div></section>
